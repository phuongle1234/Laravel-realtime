  ...
  for (var i = 0; i < numberOfOptions; i++) {
      ...
      if ($this.children('option').eq(i).is(':selected') &&  $( $this.children('option').eq(i) ).attr('selected') ){
          ...
          if(option_text = $this.children('option').eq(i).val()){
           ...
           _li = $this.parent().children('ul').find(`li[rel= '${option_text}' ]`);
            // console.log( $($this).children('ul').children(`li[rel=" ${option_text} "]`) );
...
function redirectToRequestManagement(_id){

  // let json = JSON.parse(JSON.stringify(data));
  let _token = $('meta[name=csrf-token]').attr('content');


  let updateURL = window.location.protocol+'//'+window.location.host + '/admin/notification_management/update-notification';

  $.post(updateURL,{ _token: _token, id: _id  },(output) => {
    $('.count_noti').text(output.count);
  });

  // $('#notification_modal').find('h3.title').html(json.title);
  // $('#notification_modal').find('p.content-txt').html( nl2br( json.content ) );

  //$('.dropdown-toggle').trigger('click');
  window.location.href = window.location.protocol+'//'+window.location.host + '/admin/video_management/list_approval';
