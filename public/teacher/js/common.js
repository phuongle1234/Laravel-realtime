...
$('.menu-down').click(function() {
  var select = $(this).find('.menu-child');
  $(this).toggleClass('show');
  select.css("display:none");
  if (select.height() != 0)
    select.animate({ height: "100%", opacity: 1}, 500);
  else
    select.animate({ height: 0 , opacity: 0}, 500);
});

$('.menu-down .menu-child .menu-child-item').click(function (){
  $(this).closest('.menu-child').find('.menu-child-item.active').removeClass('active');
  $(this).addClass('active');
});
...
/**
 * show slider in popup
 */

 $('.requestPopup').on('shown.bs.modal', function () {
  $('.listimgs').slick("refresh");
  if($( window ).width() > 1024){
    $( ".list-request" ).each(function() {
      $( this ).find('.request-item .request-tit').tile(4);
    });
  }
  else if($( window ).width() > 639){
    $( ".list-request" ).each(function() {
      $( this ).find('.request-item .request-tit').tile(2);
    });
  }
});
...
$('.main_body').click(function(e) {
  if($(e.target).hasClass('dropdown-info') || $(e.target).hasClass('info-box')){
      e.preventDefault();
      return;
  }
    ...
    $('.main_body').removeClass('bgtransparent');
...
if($(window).width() < 1025 ){
  $('.info-box').click(function(){
    $('.main_body').removeClass('show-menu');
...
$('.info-box, .info-icon').click(function(){
  $(this).closest('body').find('.main_body').toggleClass('bgtransparent');
});
...
$('.select-video .item').click(function () {
  $(this).parent().find('.item.active').removeClass('active');
  $(this).addClass("active");
});
...
const resizingImg =  async (h,w, url) => {
  var image = new Image();
  image.width = w;
  image.height = h;
  image.src = url;
