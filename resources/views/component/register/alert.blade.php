@if ($errors->any())
    @foreach ($errors->all() as $error)
        <div class="alert alert-dark" id="alert-failed">
                    <span>{!! nl2br($error) !!}</span>

            <button class="btn-close" type="button" onclick="$('div.alert-dark').remove()">
...
@if (session('message'))
        <div class="alert alert-dark" onclick="$('div.alert-dark').remove()">
            <span>{!! nl2br( session('message') ) !!}</span>
