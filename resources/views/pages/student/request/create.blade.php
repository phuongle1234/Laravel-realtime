...
@section('content')
  ...
  <!-- <input type="file" id="file-upload" name="file_temp" style="display:none"> -->
  ...
  <input type="file" name="path[]" data-index="4" data-chose="0" style="display:none" accept="image/*" >
                ...
                <section class="section_request_detail">
                  ...
                  <a class="back-link fsc" href="{{ url()->previous() }}">
                    <img src="{{ asset('student/images/back-icon.svg') }}" alt="{{ trans("student.request_create")  }}"><span>{{ trans("student.request_create") }}</span>
                  </a>

                  <div class="box-shawdow">
                    <h2 class="pp-request-tit">リクエスト内容</h2>

                    <div class="block-users" style="display:none">
                              <figure class="img-circle">
                                <img src="" alt="お名前太郎さん">
                              </figure>
                              <div class="users-info">
                                <h3 class="tit"></h3>
                                <p class="text">大学 : <span id="university"></span></p>
                                <div class="list-tag">
                                </div>
                              </div>
                    </div>
                      ...
                      <div class="block-top">
                        ...
                        <div class="item fiximgafter">
                          <form id="form">
                                  <input class="form-control" type="hidden" name="user_receive_id" value="" >
                            <dl>
                              <dt>タイトル</dt>
                              <dd>
                                <input class="form-control" placeholder="{{ trans('common.placeholder.title') }}" type="text" name="title" pattern="^\S+$" value="{{ old('title') }}" maxlength="100" required>
                              </dd>
                            </dl>
                            <dl>
                              <dt>説明文</dt>
                              <dd>
                                <textarea placeholder="{{ trans('common.placeholder.explanatory') }}" class="form-control" name="content" pattern="^\S+$" required maxlength="1000" >{{ old('content') }}</textarea>
                              </dd>
                            </dl>
                            <dl>
                              <dt>科目選択</dt>
                              <dd>
                                <select class="form-control" name="subject_id"  required data-embed="false">
                                  ...
                                  @foreach($subject as $key => $_sub)
                                    <option value="{{ $_sub->id }}" {{ old('subject_id') && old('subject_id') == $_sub->id ? 'selected' : null }} >
                                      {{ $_sub->text_name }}
                                    </option>
                                  @endforeach
                                </select>
                    ...
                    <!-- data-bs-toggle="modal" data-bs-target="#requestTicket" -->

                    <div class="block-bottom">

                      <button class="btn btn-image btn-primary btn-pink" data-poup="#requestTicket" onclick="show_poup(event)" >
    ...
    <!-- modal- submit -->
            <div class="modal fade popup-style popup-style1 popup-bottom-sp" id="requestTicket" tabindex="-1" role="dialog" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <button class="close" data-bs-dismiss="modal" aria-hidden="true"></button>
                  <div class="modal-body">
                    ...
                    <figure class="avatar-box"><img src="{{ asset('student/images/icon_request_001.svg') }}" alt="お名前太郎さん"></figure>
                    ...
                    <div class="btn-ticket text_center">
                      <button onclick="sendRequest()" class="btn btn-primary btn-pink" {{ ! $_user->ticket ? 'disabled' : null }} >リクエストを完了する</button>
