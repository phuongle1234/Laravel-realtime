...
@section('body_class','p-setting inquiry pdbottom hasbg')

@section('class_main','p-content-setting inquiry pdbottom')

@section('content')
<section>
    <div class="setting-wrapper"><a class="back-link fsc" href="{{route('teacher.setting.index')}}"><img src="{{asset("images/back-icon.svg")}}" alt="お問い合わせ"><span>お問い合わせ</span></a>
        ...
        <form class="setting-content" id="form-submit" method="post">
          ...
          <div class="setting-body">
            <dl class="noinline">
              <dt>お名前</dt>
              <dd>
                <input class="form-control" type="text" name="name" maxlength="12" value="{{ old('name') }}" required>
              </dd>
            </dl>
            <dl class="noinline">
              <dt>メールアドレス</dt>
              ...
              <dd>
                <div class="select">
                  <select name="options" id="options" class="select-hidden">
                    @foreach(EInquiry::TEACHER_INQUIRY_OPTIONS_ARRAY as $k => $v)
          ...
          <div class="text_center">
            <label class="text-bottom" for="check">
              <input type="checkbox" name="agreement" id="check" required><span> <a href="{{ route('teacher.setting.privacy_policy') }}"> プライバシーポリシー </a> に同意する<br>※ご同意いただけない場合は送信ができません</span>
            </label>
