...
<div class="box-top boxshadow">
  ...
  <form method="post" id="form_submit" enctype="multipart/form-data" >
    ...
    <input  name="path_id" type="hidden" value="{{ $item->path }}" >
    <input  name="video_id" type="hidden" value="{{ $item->video->id }}" >

    <div class="box-bottom boxshadow">
                  <div class="item">
                    <h3 class="green-tit">動画アップロード</h3>

                    <!-- if( strpos($item->video->player_embed_url,'transcode.svg') > 0 )
                    <figure class="video-block"><img width="478" height="281" src="{{ $item->video->player_embed_url }}" alt=""></figure>
                    else -->

                    <iframe src="{{ $item->video->player_embed_url	 }}"  width="478" height="281" frameborder="0" allowfullscreen allow="autoplay; encrypted-media"></iframe>
                    ...
                    <span class="btn btn-image btn-pdf" onclick="$('#add_path').trigger('click')" >
                        ...
                        <span id="file-pdf"><?= strlen($_file_name) > 20 ? substr($_file_name,0,20)."..." : $_file_name  ?>​</span>
                    </span>
                  ...
                  <div class="item">
                    <div class="request-detail">
                      <dl class="noinline">
                        <dt>動画タイトル</dt>
                        <dd>
                          <input class="form-control" type="text" name="video_title" maxlength="" required value="{{ $item->video_title	}}">
                        </dd>
                      </dl>
                      <dl class="noinline">
                        <dt>動画説明文</dt>
                        <dd>
                          <textarea class="form-control" name="description" cols="30" rows="5" required >{{ $item->description	}}</textarea>
                        </dd>
                      </dl>
                      <dl class="noinline">
                        <dt>科目</dt>
                      ...
                      <dl class="noinline">
                        ...
                        <dd>
                          ...
                          @foreach( $item->subject->tags as $_key => $_val )
                            <li>
                              <input type="radio" name="tag_id" value="{{ $_val->id }}" required {{ $item->tag_id == $_val->id ? 'checked' : null }} >
                              <div><span>{{ $_val->name }}</span></div>
                            </li>
                          @endforeach
                          </ul>
                        </dd>
                        ...
                        <dd>
                          <ul class="list-courses list-courses-red">
                            @foreach( $Tag::where('tag_type','difficult')->get() as $_row )
                              <li>
                                <input type="radio" name="field_id" value="{{ $_row->id }}" required {{ $item->field_id == $_row->id ? 'checked' : null }} >
...
<script>
  ...
  $(document).ready(function(){
    ...
    // if( _iframe.getAttribute('src').search( 'transcode.svg' ) < 0)
     ...
     if( e.target.src.search( 'transcode.svg' ) >= 0 )
       e.target.src = await resizingImg( e.target.height , e.target.width , e.target.src);
  ...
  function showTag(_id,f){
      ...
      let tag_filter = _tag.filter(item => parseInt(item.subject_id) == parseInt(_id) );
        ...
        $.each(tag_filter , (e,val) => {
            $('#tag_id').append(`
                <li>
                  <input type="radio" name="tag_id" value="${val.id}">
