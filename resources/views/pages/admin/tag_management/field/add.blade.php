                ...
                <table>
                  <tbody>
                        ...
                        <div class="select-box">
                          <select name="subject_id" class="form-control">
                            ...
                            @foreach($subject::all() as $key => $row)
                              <option value="{{ $row->id }}" {{ old('subject_id') == $row->id ? 'checked' : null  }} >{{ $row->text_name }}</option>
                            @endforeach
                          </select>
                        </div>
                      </td>
                    ...
                    <tr>
                      <th>タグ名</th>
                      <td>
                        <input name="name" class="form-control" type="text" maxlength="20" value="{{ old('name') }}" required>
                        <div class="choose-subject mt30" id="active">
                          <div data-id="1" class="item {{ old('active') == 1 || ! old('active')  ? 'active' : null  }}" >表示</div>
                          <div data-id="0" class="item {{ old('active') == 0 && is_numeric( old('active') )  ? 'active' : null  }}">非表示</div>
                        </div>
                      </td>
                    </tr>
                  </tbody>
