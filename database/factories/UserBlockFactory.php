...
class UserBlockFactory extends Factory
{
    ...
    public function definition()
    {
        ...
        $_block_user_id	 = collect( User::where('role','teacher')->get()->toArray() )->map( function($val,$inx){ return $val['id'];  } )->toArray();

        return [
            'user_id' => $this->faker->randomElement( $_user_id ),
            'blocked_user_id' => $this->faker->randomElement( $_block_user_id )
        ];
    }
}
